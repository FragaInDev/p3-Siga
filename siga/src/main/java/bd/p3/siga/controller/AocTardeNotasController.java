...
@Controller
public class AocTardeNotasController {
	
	@Autowired
	private NotasDAO nDao;
	
	@RequestMapping(name = "aocTN", value = "/aocTN", method = RequestMethod.GET)
	public ModelAndView init(ModelMap model) {
		return new ModelAndView("aocTN");
	}
	
	@RequestMapping(name = "aocTN", value = "/aocTN", method = RequestMethod.POST)
	public ModelAndView notas(@RequestParam Map<String, String> allParam, ModelMap model) {
		...
		try {
			if (botao.equalsIgnoreCase("listar")) {
				...
				alunos = nDao.findNotas(sigla, turno);
